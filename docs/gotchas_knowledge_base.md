...
### 5.13 Subagent scope creep — chỉ định rõ phạm vi + không để dirty state
- **Vấn đề:** Subagent được dispatch cho Task 4 (migrate promotions page) tự ý sửa thêm settings, tables, analytics, platform-admin/menu pages mà không commit, để lại dirty state. Các sửa đổi cũng sai (dùng `themeConfig` object thay vì flat fields trong updateTheme mutation), gây lỗi build.
- **Cách khắc phục:** Khi dispatch subagent, chỉ định rõ ràng "chỉ sửa file X, không đụng file khác". Nếu subagent báo DONE nhưng còn dirty state → revert và re-dispatch. Luôn kiểm tra `git status` sau mỗi subagent.
